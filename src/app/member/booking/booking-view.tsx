...
export function BookingView() {
        ...
        <main className="space-y-4 px-4 pb-24 pt-8">
          ...
          <Card className="divide-y divide-ink/8">
            <div className="flex items-center justify-between px-4 py-3">
              <span className="text-xs text-ink/55">日時</span>
              <span className="text-sm font-bold">
                {selected.dayLabel} {selected.time}〜 (60分)
              </span>
            </div>
            <div className="flex items-start justify-between gap-3 px-4 py-3">
              <span className="shrink-0 text-xs text-ink/55">ご相談内容</span>
              <span className="text-right text-sm">{purpose}</span>
            </div>
            <div className="px-4 py-3">
              <p className="text-xs text-ink/55">Zoomリンク</p>
              <p className="mt-1 break-all rounded-sm bg-cream px-3 py-2 font-mono text-xs text-brand-dark">
                {zoomUrl}
              </p>
            </div>
          </Card>
          ...
          <div className="space-y-2 pt-2">
            <Link
              href="/member"
              className="block w-full rounded-sm bg-brand py-3.5 text-center text-sm font-bold text-white transition-colors active:bg-brand-dark"
            >
              ホームへ戻る
            </Link>
            <button
              onClick={reset}
              className="w-full rounded-sm border border-ink/20 py-3.5 text-sm font-bold text-ink/70"
            >
              別の枠を予約する
            </button>
          </div>
  ...
  return (
    <>
      <PageHeader title="Zoom予約" backHref="/member" />
      <main className="space-y-4 px-4 pb-24 pt-4">
        ...
        {slots === null ? (
          <div className="flex flex-col items-center gap-3 py-16">
            <span className="h-8 w-8 animate-spin rounded-full border-2 border-ink/12 border-t-brand" />
          ...
          <>
            ...
            <section className="space-y-3">
              {dayLabels.map((day) => (
                <Card key={day} className="p-4">
                  <p className="text-xs font-bold text-ink/55">{day}</p>
                        ...
                        return (
                          <button
                            ...
                            className={`min-h-11 rounded-sm border px-4 py-2 text-sm font-semibold transition-colors ${
                              !s.available
                                ? "border-ink/12 bg-ink/5 text-ink/25 line-through"
                                : isSelected
                                  ? "border-brand bg-brand text-white shadow-md shadow-brand/30"
                                  : "border-ink/20 bg-white text-ink/80 active:bg-brand-soft"
                            }`}
                          >
                            {s.time}
                          </button>
            ...
            <Card className="p-4">
              <label
                ...
                className="text-xs font-bold text-ink/55"
              >
                ご相談内容 (目的)
              </label>
              <textarea
                ...
                className="mt-2 w-full resize-none rounded-sm border border-ink/12 bg-cream px-3 py-2.5 text-sm outline-none focus:border-brand focus:bg-white"
              />
              {selected && (
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-sm bg-brand py-3.5 text-sm font-bold text-white transition-colors active:bg-brand-dark disabled:opacity-40 disabled:shadow-none"
            >
              {phase === "sending" ? (
                <>
