...
import { currentMember, memberTasks } from "@/lib/mock-data";
import { MemberPushToggle } from "./push-toggle";
        ...
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-lg font-bold">{currentMember.name}</p>
              <p className="text-xs text-ink/55">
                作業者(クリエイター) / {formatDate(currentMember.joinedAt)} 登録
              </p>
              <div className="mt-1.5 flex flex-wrap gap-1">
                {(currentMember.specialties ?? []).map((s) => (
                  <span
                    key={s}
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 border-t border-ink/8 pt-3">
            <div className="text-center">
              ...
              <p className="text-base font-bold text-brand">
                {currentMember.completedCount}件
              </p>
            </div>
            <div className="text-center">
